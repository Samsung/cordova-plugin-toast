...
module.exports = {
	setSource: function (success, fail, args){
		...
		if (!args[0].type || !args[0].number) {
			throw error('TypeMismatchErr', 'Failed to find the source.');
		}
	...
	show: function (success, fail, args) {
		...
		if (args[0].length != 4) {
			throw error('TypeMismatchErr', 'Rectangle values are wrong.');
		}
	...
	hide: function (success, fail, args) {
		...
		if (!element) {
			throw error('InvalidStateErr', 'There are no window.');
		}
