...
Game = {
  compatible: function() {
	...
	stop: function(game){
		if (game.runner){
			window.cancelRequestAnimFrame(game.runner.timer);
			return {};
		}
	},
    ...
    if(typeof callback === 'function'){
      Breakout.onabandon = callback;
    } else {
      Breakout.onabandon = function(){
        game = null;
      };
    }
    
    // Sets the variables to initialize. 
    var id = 'canvas',
    game = Breakout,
    cfg = '';
    
    //Attacht the CSS stylesheet.
    var head  = document.getElementsByTagName('head')[0];
    var link  = document.createElement('link');
    link.rel  = 'stylesheet';
    link.type = 'text/css';
    link.id = 'brekout_id';
    link.href = 'app/javascripts/samsung/breakout.css';
    link.media = 'all';
    head.appendChild(link);

    

    var b = document.getElementById("canvasdiv");
      c = document.getElementById("canvas");
      
    if(document.body.clientWidth > 1200 ) {
      b.className = b.className + " res_720px";
      c.className = c.className + " res_720px";
    } else {
      b.className = b.className + " res_540px";
      c.className = c.className + " res_540px";
    }
    ...
    setTimeout(function(){
      if (Game.compatible()){
        $('#canvasdiv').addClass('e_in').css('visibility', 'visible').on('webkitAnimationEnd', function(){
          $('#canvasdiv').removeClass('e_in').off();
		  Breakout.Defaults.keys = breakout_keys;
        });
      }
    },250);
  ...
  windowWidth:  function() { return 1280  },
  windowHeight: function() { return 720 },
  ...
  Runner: {
    ...
    start: function() { // game instance should call runner.start() when its finished initializing and is ready to start the game loop
      ...
      this.timer     = setTimeout(this.loop.bind(this), this.interval);//setInterval(this.loop.bind(this), this.interval);
    },

    loop: function() {
      this.timer = window.requestAnimFrame(this.loop.bind(this));
